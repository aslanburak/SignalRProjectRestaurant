...
namespace SignalRWebUI.Controllers
{
	public class AboutController : Controller
	{
		...
		public async Task<IActionResult> Index()
		{
			...
			var responseMessage = await client.GetAsync("https://localhost:7031/api/About"); // API URL'si

			if (responseMessage.IsSuccessStatusCode)
			{
				var jsonData = await responseMessage.Content.ReadAsStringAsync(); // JSON verisini oku
				var values = JsonConvert.DeserializeObject<List<ResultAboutDto>>(jsonData); // JSON verisini nesnelere dönüştür

				//JsonConvert.DeserializeObject<List<ResultAboutDto>>(jsonData) → JSON string’ini List<ResultAboutDto> nesnelerine dönüştürür.
				//Buradaki ResultAboutDto sınıfı JSON alanlarıyla eşleşmelidir.
				return View(values);
			}
			return View();
		}
		...
		public async Task<IActionResult> CreateAbout(CreateAboutDto createAboutDto)
		{
			...
			var jsonData = JsonConvert.SerializeObject(createAboutDto); // DTO nesnesi JSON string'ine dönüştürülür
			StringContent stringContent = new StringContent(jsonData, Encoding.UTF8, "application/json"); // JSON içeriği hazırlanır

			var responseMessage = await client.PostAsync("https://localhost:7031/api/About", stringContent); // POST isteği gönderilir
			if (responseMessage.IsSuccessStatusCode)
			{
